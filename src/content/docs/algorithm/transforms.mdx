...
r &= \sqrt{uv.x^2 + uv.y^2}, \\
\theta &= \mathrm{atan2}(uv.y,\ uv.x), \\
\theta' &= \theta + \mathrm{amount}\cdot r^\beta, \\
\text{uv}' &= \bigl(r\cos\theta',\;r\sin\theta'\bigr)
\end{aligned}
$$
...
\begin{aligned}
\theta &= \mathrm{atan2}(uv.y,\ uv.x), \\
\theta' &= \cos(\sin(\theta)\cdot \mathrm{freq1}) \cdot \mathrm{freq2}, \\
\text{uv}' &= \bigl(\,uv.x,\; uv.y + \mathrm{amp}\cdot\theta'\bigr)
\end{aligned}
$$
